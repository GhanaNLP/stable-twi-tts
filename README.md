...
| **Native port + espeak** | matches Python | ~30 lines of code + 1.56 MB — unlimited vocabulary, no Python |
...
20,000 words. So a Twi-only app needs no Python, no espeak and no lexicon. English is not portable
that way (7,132 context rules plus 5,794 exceptions) and should link `libespeak-ng`, which is
1.56 MB with English-only data.
